...
function add_back(id, pos){
    element = document.getElementById(id);
    element.style.display = "none";
    pos = id.substr(8, 2);
    if (segment == 0){
        selected_ids.first = pos;
        document.getElementById("topright").style.backgroundImage = "url(/pictures/config/backgrounds/background" + pos + ".jpg)";
    }
    if (segment == 1){
        selected_ids.second = pos;
        document.getElementById("bottomright").style.backgroundImage = "url(/pictures/config/backgrounds/background" + pos + ".jpg)";
    }
    if (segment == 2){
        selected_ids.third = pos;
        document.getElementById("bottomleft").style.backgroundImage = "url(/pictures/config/backgrounds/background" + pos + ".jpg)";
    }
    if (segment == 3){
        selected_ids.fourth = pos;
        document.getElementById("topleft").style.backgroundImage = "url(/pictures/config/backgrounds/background" + pos + ".jpg)";
    }
    segment++;
    if (segment == 4){
